...
Javier, you might want to write some sentences about how you did this part of the assignment. 
...
I think ehab did this part so write a little summary here about how you wrote this and why you did it. 
...
Now,  we understand that it is not necessary to use 2 factors, since the results from using 1 factor tells us that all the variables are correlated with each other. However, to understand if the oblique roation using 2 factors was necessary we need to get the full summary. 
```{r}
pre_opt <- pca(r = pre, nfactors = 2, rotate = "oblimin")
pre_opt

pre_opt <- pca(r = res, nfactors = 2, rotate = "oblimin")
pre_opt
...
res_obl
res_obl <- pca(r = res, nfactors = 2, rotate = "oblimin")$loadings[]
res_obl


pre_olb <- pca(r = pre, nfactors = 1, rotate = "oblimin")$loadings[]
pre_olb
pre_olb <- pca(r = pre, nfactors = 2, rotate = "oblimin")$loadings[]
pre_olb
